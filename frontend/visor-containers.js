/********************************************************************************
 * [MODULE]: VISOR DE CONTENEDORES Y BINDERS (visor-containers.js)
 * [DESCRIPTION]: Lógica de renderizado 2D/3D, lupa 2.5D, gestión de páginas, 
 *                movimiento de cartas (interno y cross-container) y slots.
 ********************************************************************************/
...
document.addEventListener('DOMContentLoaded', () => {
    /****************************************************************************
     * 1. DECLARACIÓN DE SELECTORES Y ESTADOS GLOBALES
     ****************************************************************************/
    ...
    /****************************************************************************
     * 2. LECTURA DE PARÁMETROS DE URL (INICIALIZACIÓN DE CARTA / VISTA)
     ****************************************************************************/
...
// --- 3. CARGA DE CONTENEDORES ---
    async function cargarContenedores() {
        try {
            ...
            // Si viene un container_id por la URL (igual que haces con las cartas), lo seleccionamos al momento
            const paramContainerId = new URLSearchParams(window.location.search).get('container_id');
            if (paramContainerId && containerSelector) {
                const existe = allContainers.some(c => c.id === paramContainerId);
                if (existe) {
                    containerSelector.value = paramContainerId;
                    containerSelector.dispatchEvent(new Event('change'));
                }
            }
        } catch (err) {
