...
// --- Conexión con Scryfall para obtener carta aleatoria ---
async function cargarCartaDesdeApi() {
    try {
...
// Inicializar el tamaño del fondo de la lupa cuando cargue la imagen de la API
cardImage.addEventListener('load', () => {
    magnifier.style.backgroundSize = `${card.offsetWidth * ZOOM_SCALE}px auto`;
});


// --- Evento principal de movimiento (2.5D + Lupa conmutable) ---
container.addEventListener('mousemove', (e) => {
    const rect = container.getBoundingClientRect();
    const mouseX = e.clientX - rect.left;
    const mouseY = e.clientY - rect.top;

    // Lógica de la lupa (solo si está activada)
    if (isMagnifierEnabled) {
    ...
    // Lógica de inclinación 2.5D (funciona siempre)
